...
function handleSearch(inputEl) {
  ...
  if (location.pathname + location.search !== url) {
    history.replaceState({view:'home', q: val}, '', url);
  }
}
...
function renderCountry(countryId, skipPush) {
  ...
  appContainer().innerHTML =
    ...
    '</div>' +
    '</header>' +
    '<div class="pt-6 fade-in">' + cardsHtml + '</div>';
  ...
  appContainer().innerHTML =
    ...
    (function(){
      var srcs=[
        ...
        {url:item.sourceUrl4,name:item.source4,snip:item.sourceSnippet4},
        {url:item.sourceUrl5,name:item.source5,snip:item.sourceSnippet5}
      ];
      ...
      srcs.forEach(function(s){
        ...
        h+='<a href="'+s.url+'" target="_blank" rel="noopener" class="block p-4 border border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">'
          ...
          +'<span style="position:relative;display:inline-block;width:2.5rem;height:2.5rem;flex-shrink:0;margin-top:0.125rem">'+'<img src="https://www.google.com/s2/favicons?domain='+d+'&sz=64" style="width:2.5rem;height:2.5rem;border-radius:0.25rem;border:1px solid #e5e7eb;background:#fff;position:absolute;top:0;left:0" onerror="this.style.opacity=\'0\';this.nextElementSibling.style.display=\'flex\'">'+'<span style="display:none;width:2.5rem;height:2.5rem;border-radius:0.25rem;border:1px solid #e5e7eb;background:#dbeafe;color:#2563eb;font-weight:700;font-size:0.875rem;align-items:center;justify-content:center;position:absolute;top:0;left:0">'+(s.name||d).charAt(0).toUpperCase()+'</span></span>'
